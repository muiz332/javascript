...
jadi ada dua cara untuk menterjemahkan source code yang sudah kita buat kedalam bahasa mesin agar bisa kita jalankan 
ada source code yang harus kita compile menggunakan sesuatu yang namannya compailer ada yang harus kita terjemankan 
atau kita interpretasi menggunakan interpreter 

nah sebetulnya kita tidak perlu khawatir bagaimana kita melakukan dua hal ini karena tiap tiap bahasa pemrogaman sudah menyediakan cara untuk 
melakukan hal tersebut dan pada umumnya dilakukan secara otomatis tapi tidak ada salahnya jika kita tahu cara kerjanya dan perbedaanya 
...
- COMPILER
contohnya begini misalnya kita anggap saja ada dua komputer  A dan B, nah si A ini ceritannya membuat progam nanti progamnnya akan diberikan pada 
si B jika menggunakan compiler maka ini yang akan terjadi
...
dan juga compiler ini ketika kita jalan jika ada code yang error maka dia tidak bisa dicompiler atau
tidak dapat diterjemahkan menjadi bahasa mesin
...
- INTERPRETER 
si A membuat sebuah source code, tapi sekarang si A tidak akan melakukan compile melainkan dia akan menduplikat source codenya (mengcopy) untuk
diberikan kepada si B, jadi source codenya yang diberikan kepada si B atau pada usernnya, nanti si B ini harus menggunakan sebuah interpreter untuk
menterjemahkan progamnya si A, lalu dia akan menjalankan interpreternya sekaligus menjalankan progamnya, jadi interpreter ini berjalan real time pada
saat menterjemahkan source codenya baris per baris 

jadi misalkan yang error ada dibaris ke 10, code sebelum baris ke 10 itu akan tetap dijalankan,dan baris yang setelah 10 dia tidak akan
dijalankan
...
kelebihannya 
    ...
    3.source code tidak terlihat 
      jadi source codenya akan lebih aman karena yang dilihat oleh user adalah hasil compilenya bukan source codenya, jadi source codenya tetap 
      ada diprogammernya 
...
nah ternyata ada beberapa bahasa yang menggabungkan kedua tehnik tersebut, tekniknya disebut dengan teknik Hybrid atau bahasanya ada yang 
...
nah setelah itu intermedia language ini diberikan kepada si B dan si B melakukan langkan terakhir untuk menjalankan progam itu dengan sesuatu 
yang dinamankan dengan JIT compilation(just in time compilation) 
...
nah sebatulnya secara teori semua bahasa pemrogaman itu bisa dijalankan menggunakan ketiga cara tadi tapi biasanya pada tiap tiap bahasa yang 
di gunakan hanya salah satunya saja 

nah berikut ini adalah beberapa contoh metode yang digunakan pada bahasa pemrograman
...
simplenya gini jadi jika kalian mau bahasa yang progamnya cepat maksutnya teroptimasi untuk bahasa mesin pada cpu tertntu gunakan bahasa 
yang memiliki compiler kalo mau fleksible bisa digunakan diberbagai sistem opertasi gunakan bahasa yang memiliki interpreter 
